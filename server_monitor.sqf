...

{
	...
	*/
		...
		if (call GTS_BankObjectUpdateCond) then {
...
if (_hiveLoaded) then {
	_serverVehicleCounter spawn {
		...
		if (GTS_DrugsSystem && {GTS_WeedFarmsDynSpawn}) then {
			diag_log ("HIVE: Spawning # of Weed Farms: " + str(MaxWeedFarms));
			for "_x" from 1 to MaxWeedFarms do {call spawn_weedFarms;};
		};
...
if (GTS_SafezoneRelocate) then {
	execVM "\z\addons\dayz_server\addons\safeZoneRelocate.sqf";
};