...
"_nearLight","_vehicleOwnerID","_hasHotwireKit","_allowedDistance","_id","_upgrade","_weaponsPlayer","_hasCrowbar",
"_allowed","_hasAccess","_uid","_myCharID","_isLocked","_isDistance","_door","_doors",
"_vOwner","_vFriends","_vAllowed","_vOwner","_vFriends","_vAllowed"];
...
if (typeName _vehicleOwnerID != "STRING") then { _vehicleOwnerID = str(_vehicleOwnerID) };
...
if (GTS_AttachZeds) then {
	_allowedVehiclesA = typeOf _vehicle in GTS_vehiclesToAttachZeds;

	if (_inVehicle && _allowedVehiclesA && (driver _vehicle == player)) then {
		DZE_myVehicle = _vehicle;
		if (s_player_zedsr < 0) then {
			s_player_zedsr = DZE_myVehicle addAction ["Отпустить зомби","scripts\attachZeds\detachZed_veh.sqf",DZE_myVehicle,2,false,true,"",""];
		};
	} else {
	...
	if (GTS_GarageDoorOpener) then {
		...
		if (count _doors > 0 && {driver DZE_myVehicle == player}) then {
			...
			if (s_player_gdoor_opener_ctrl < 0 && ((_hasAccess select 0) || (_hasAccess select 2) || (_hasAccess select 3) || (_hasAccess select 4))) then {
				_door = DZE_myVehicle addAction [localize "STR_GTS_GARAGE_DOOR_OPENER","scripts\garageDoorOpener.sqf",_doors select 0, 1, false, true];
...
//Has some kind of target
	...
	if (typeName _characterID != "STRING" ) then { _characterID = str(_characterID) };
	...
	if (_isVehicle && {!_isMan} && {dayz_myCursorTarget != _cursorTarget} && {_hasToolbox} && {damage _cursorTarget < 1} && {_typeOfCursorTarget != "M240Nest_DZ"}) then {
		if (s_player_repair_crtl < 0) then {
			...
			_menu = dayz_myCursorTarget addAction [localize "str_actions_repairveh", "custom\actions\repair_vehicle.sqf",_cursorTarget, 0, true, false];
			if (!_isBicycle) then { //Bike wheels should not give full size tires. Also model does not update to show removed wheels.
				if (!DZE_salvageLocked) then {
					if (!_isLocked) then {
						_menu1 = dayz_myCursorTarget addAction [localize "str_actions_salvageveh", "custom\actions\salvage_vehicle.sqf",_cursorTarget, 0, true, false];
					...
					if (s_player_evacChopper_ctrl < 0) then {
						...
						if (_hasKey || _oldOwner) then {
							if ((_cursorTarget isKindOf "Helicopter") && (!playerHasEvacField)) then {
								_setEvac = player addAction [("<t color=""#0000FF"">" + localize "STR_GTS_SET_JAEM" + "</t>"),"scripts\JAEM\SetEvacChopper.sqf",_cursorTarget,3,false,false];
							...
							if ((_cursorTarget isKindOf "Helicopter") && playerHasEvacField && (player distance playersEvacField < 10)) then {
								_clearEvac = player addAction [("<t color=""#0000FF"">" + localize "STR_GTS_CLEAR_JAEM" + "</t>"),"scripts\JAEM\ClearEvacChopper.sqf",_cursorTarget,4,false,false];
	...
	// Hotwire Vault
	if (GTS_HotwireVault) then {
		if (_typeOfCursorTarget in DZE_LockableStorage) then {	
			if (s_player_hotwirevault < 0) then {                 	 
				s_player_hotwirevault = player addaction [("<t color=""#B40404"">" + localize "STR_GTS_HOTWIRE_VAULT" + "</t>"), "scripts\hotwireVault.sqf",_cursorTarget, 0, false, true, "", ""];
			};				   
		} else {
	...
	_vOwner = _cursorTarget getVariable ["ownerPUID","0"];
	_vFriends = _cursorTarget getVariable ["VaultFriends",[]];
	_vAllowed = [_vOwner];
	{
	    _vAllowed set [(1+_forEachIndex), (_x select 0)];
	} forEach _vFriends;
	...
	if ((_typeOfCursorTarget in DZE_LockableStorage) && {_characterID != "0"} && {_isDistance} && {!keypadCancel}) then {
		if (s_player_unlockvault < 0) then {
			if (_typeOfCursorTarget in DZE_LockedStorage) then {
				if (((getPlayerUID player) in _vAllowed) && s_player_vaultma < 0) then {
					s_player_vaultma = player addAction ["<t color='#32b4c1'>Manage Vault</t>", "scripts\vm\initVaultManagement.sqf", _cursorTarget, 5, false];
				};
	...
	if ((damage _cursorTarget >= DZE_DamageBeforeMaint) && {_cursorTarget isKindOf "ModularItems" || _cursorTarget isKindOf "DZE_Housebase" || _typeOfCursorTarget == "LightPole_DZ"}) then {
		...
		if (s_player_maint_build < 0) then {
			_hasAccess = [player, _cursorTarget] call FNC_check_access;
			if ((_hasAccess select 0) or (_hasAccess select 2) or (_hasAccess select 3)) then {
				_text2 = _text + " (" + str(round ((damage _cursorTarget) * 100)) + localize "STR_GTS_BUILDING_DAMAGED";
	...
	if (call GTS_BankInteractAction) then {
	...
	if (GTS_BuryAndButcherBodies) then {
			...
			if (!_isButchered) then {
				if ("ItemEtool" in _itemsPlayer) then {
					if (s_player_bury_human < 0) then {
						s_player_bury_human = player addAction ["<t color='#0096ff'>" + localize "STR_GTS_BURY_HUMAN" + "</t>","scripts\buryActions.sqf",[_cursorTarget,"bury"],0,false,true];
					}
				} else {
				...
				if ({_x in ["ItemKnife","ItemKnife5","ItemKnife4","ItemKnife3","ItemKnife2","ItemKnife1"]} count _itemsPlayer > 0) then {
					if (s_player_butcher_human < 0) then {
						s_player_butcher_human = player addAction ["<t color='#0096ff'>" + localize "STR_GTS_BUTCHER_HUMAN" + "</t>","scripts\buryActions.sqf",[_cursorTarget,"butcher"],0,false,true];
					};
				} else {
	...
	if (GTS_WalkAmongstTheDead) then {
		if (!_isAlive && {(_cursorTarget isKindOf "zZombie_base")}) then {
			...
			if (!_wasguted) then {
				if (s_player_zhide2 < 0) then {
					s_player_zhide2 = player addAction ["<t color='#0096ff'>" + localize "STR_GTS_SMEAR_GUTS" + "</t>", "scripts\wad\smear_guts.sqf",_cursorTarget,0, false,true];
				};
			} else {
	...
	if (GTS_TakeClothes) then {
		if (_isMan && {!_isAlive} && {!(_cursorTarget isKindOf "zZombie_base")} && {!(_cursorTarget getVariable["clothesTaken",false])}) then {
			if (s_player_clothes < 0) then {
				s_player_clothes = player addAction ["<t color='#0096ff'>" + localize "STR_GTS_TAKE_CLOTHES" + "</t>", "scripts\takeClothes.sqf",_cursorTarget,0, false,true];
			};
		} else {
		...
		if (_isAlive && {(_cursorTarget isKindOf "zZombie_base")}) then {
			...
			if (!_zattached) then {
				if ("equip_rope" in magazines player) then {
					if (s_player_zhide4 < 0) then {
						s_player_zhide4 = player addAction [format["<t color='#0096ff'>Привязать зомби</t>"], "scripts\attachZeds\attachZed.sqf",_cursorTarget,0, false,true];
					};
				} else {
			...
			if (_zattached) then {
				if (s_player_zhide5 < 0) then {
					s_player_zhide5 = player addAction [format["<t color='#0096ff'>Отвязать зомби</t>"], "scripts\attachZeds\detachZed.sqf",_cursorTarget,0, true,true];
				};
			} else {
	...
	if (_isMan && {!(isPlayer _cursorTarget)} && {_typeOfCursorTarget in serverTraders}) then {
		if (s_player_parts_crtl < 0) then {
			...
			if ((_traderMenu select 2) == "friendly") then {
				_humanity_logic = (_humanity < GTS_LowHumBound);
			};
			...
			if ((_traderMenu select 2) == "hero") then {
				_humanity_logic = (_humanity < GTS_HighHumBound);
			};
			if (_humanity_logic) then {
				...
				// Static Menu
				{
					_buy = player addAction [format[localize "STR_GTS_TRADE_IT_FOR",(_x select 3),(_x select 5),(_x select 2),(_x select 6)], "\z\addons\dayz_code\actions\trade_items_wo_db.sqf",[(_x select 0),(_x select 1),(_x select 2),(_x select 3),(_x select 4),(_x select 5),(_x select 6)], (_x select 7), true, true];
	...
	s_player_manageDoor = -1;
	// VM
	player removeAction s_player_vaultma;
	s_player_vaultma = -1;
